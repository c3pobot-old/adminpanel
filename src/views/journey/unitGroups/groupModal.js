...
export default function EditModal ({open, setOpen, saveGroup, group}){

  const [ rarityValue, setRarityValue ] = useState(group?.rarity || 1)
  const [ gearValue, setGearValue ] = useState(group?.gear?.name === 'gear' ? group.gear.value:1);
  const [ relicValue, setRelicValue ] = useState(group?.gear?.name === 'relic' ? (group.gear.value - 2):0);
  const [ numUnits, setNumUnits ] = useState(group?.numUnits || 4)
  const [ gp, setGp ] = useState(group?.gp || 0)
  ...
  return (
    <Modal open={open} onClose={handleClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
      ...
      <Table>
        <TableHead>
          <TableRow><TableCell colSpan="2"><Typography>{group?.id || 'Group Defaults'}</Typography></TableCell></TableRow>
        </TableHead>
        <TableBody>
          <TableRow>
            <TableCell colSpan="2">
